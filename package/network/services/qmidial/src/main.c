...
int set_cfun_count = 0;

static void record2file(char *file,char *buf,const char *mode)
{
	...
	if(ret != len)
		printf("WARN!write file %s!acture size %d<%d\n",file,ret,len);
	fclose(fp);
}
...
static void usbnet_link_change(int link, PROFILE_T *profile) {
    ...
    if (link) {
		set_cfun_count = 0;
		record2file("/tmp/cfuncount","QMI OK\n","w+");
...
#define has_more_argv() ((opt < argc) && (argv[opt][0] != '-'))
	...
	record2file("/tmp/cfuncount","QMI init\n","a+");
#ifdef CONFIG_BACKGROUND_WHEN_GET_IP
    ql_prepare_daemon();
#endif
                            ...
                            if (QWDS_PKT_DATA_CONNECTED != IPv4ConnectionStatus)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (abs(nowTime.tv_sec - resetRadioTime.tv_sec) > CONFIG_RESET_RADIO) {
										...
										set_cfun_count++;
										if(set_cfun_count < 5) {
											requestSetOperatingMode(0x01); //same as AT+CFUN=4,disable ME
											alarm_sec = 5;
											record2file("/tmp/cfuncount","cfun=4\n","a+");
										} else if(set_cfun_count < 9) {
											requestSetOperatingMode(0x06); //same as AT+CFUN=0,minimum func
											alarm_sec = 20;
											record2file("/tmp/cfuncount","cfun=0\n","a+");
										} else {
											//should power off ?
											set_cfun_count = 0; //again
										}
                                        requestSetOperatingMode(0x00); //same as AT+CFUN=1
                                    }
#endif
                                    alarm(alarm_sec); //try to setup data call 'alarm_sec' seconds later
                                }
                            }
                        break;
